...
gulp.task('beforeAnything', () => {
  ...
  gulp.src('./src/client/assets/**/*')
    .pipe(gulp.dest('./www/client/assets'))
...
gulp.task('build-server', ['lint', 'flowtype'], () => {
  gulp.src([paths.srv])
    .pipe(babel())
...
gulp.task('build-client', ['flowtype', 'lint', 'lint-css'], () => {
...
gulp.task('sync', ['clean', 'build-client', 'build-server'], () => {
